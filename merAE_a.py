...
class MerAE_A(nn.Module):
	def __init__(self, batch_size = 16):
		super(MerAE_A,self).__init__()
...
if __name__ == "__main__":
	...
	for epoc in range(nepoc):
		...
		with trange(len(dataloader)) as t:
			for idx in t:
				...
				arousal = sample['arousal'].to(device).type(torch.cuda.FloatTensor)

				a_hat = model(audio)

	            # loss function computation
				loss = criterion(a_hat,arousal) # MSE loss
		...
		with trange(len(valloader)) as tv:
			for idx2 in tv:
				...
				arousalv = samplev['arousal'].to(device).type(torch.cuda.FloatTensor)
				model.eval()
				with torch.no_grad():
					a_hatv = model(audiov)
					lossv = criterionv(a_hatv,arousalv)
	...
	plt.title("arousal loss training set")
	plt.xlabel('epoch number')
	plt.ylabel('MSE loss')

	plt.subplot(1,2,2)
	plt.plot(epocMetricsv['lossv'])
	plt.title("arousal loss validation set")
	plt.xlabel('epoch number')


	plt.savefig("AEbest.png")
	plt.gcf().clear()
